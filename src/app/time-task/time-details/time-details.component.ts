...
export class TimeDetailsComponent implements OnInit {
  ...
  onSubmit(form: NgForm) {
    if (this.url_id == 0) {
      ...
      form.value['id'] = this._timeService.timeHeader.length;
      for (let i = 0; i < this._timeService.timeHeader.length; i++) {
        if (this.url_id == this._timeService.timeHeader[i].id) {
          let index = this._timeService.timeHeader.indexOf(this._timeService.timeHeader[i]);
          this._timeService.timeHeader.splice(index, 1, form.value)

        }
      }

    }
    console.log(this._timeService.timeHeader);
